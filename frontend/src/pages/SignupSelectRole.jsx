...
const RoleCard = ({ icon: Icon, title, description, onClick, selected }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center gap-4 p-5 rounded-xl border-2 transition-all text-left ${selected ? "border-blue-500 bg-blue-50" : "border-gray-200 bg-white hover:border-blue-300"
      }`}
  >
  ...
  return (
    <>
      <div className="text-right">
        ...
        <Link to="/login" className="text-blue-500 font-medium text-md hover:underline">
          Sign In
        </Link>
      </div>
